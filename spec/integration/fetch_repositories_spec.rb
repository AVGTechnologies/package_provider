...
describe 'Repository Fetcher integration' do
  ...
  before(:each) do
    allow(PackageProvider::RepositoryConfig).to receive(:repositories).and_return(config)
  ...
  it 'it calls fetch on all unique directories' do
    ...
    expect(Open3).to receive(:capture3).with({}, 'git fetch --all', chdir: test_dir)
      .and_return([nil, nil, status])

    expect(Open3).to receive(:capture3).with({}, 'git fetch --all', chdir: test_dir2)
      .and_return([nil, nil, status])
